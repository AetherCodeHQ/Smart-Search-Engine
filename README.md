...
> Smart-Search-Engine - Open source tool by AetherCodeHQ

`cli` `developer-tools` `engine` `golang` `search`

## What is it?

**Smart Search Engine** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Smart-Search-Engine.git
cd Smart-Search-Engine
go build -o Smart-Search-Engine .
```

### Run
```bash
./Smart-Search-Engine --help
```
...
```bash
# Basic usage
./Smart-Search-Engine --path ./target

# With options
./Smart-Search-Engine --path ./target --format json --output report.json

# Verbose mode
./Smart-Search-Engine --path ./target --verbose
```
...
### Basic scan
```bash
./Smart-Search-Engine --path ./my-project
```

### JSON report
```bash
./Smart-Search-Engine --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Smart Search Engine
  run: ./Smart-Search-Engine --path . --format json --output report.json
```
